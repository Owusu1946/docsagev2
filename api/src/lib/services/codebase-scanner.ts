...
export type ProgressCallback = (phase: string, current: number, total: number, detail?: string) => void;
...
export class CodebaseScanner {
    ...
    async analyze(onProgress?: ProgressCallback): Promise<CodebaseAnalysis> {
        ...
        const analyzeFile = async (filePath: string): Promise<FileNode | null> => {
            try {
                ...
                return { ...node, lineCount } as FileNode & { lineCount: number };
            } catch {
                analyzed++;
                return null;
            }
        ...
        const results = await Promise.all(
            filesToAnalyze.map(file => this.limit(() => analyzeFile(file)))
        );
        ...
        for (const result of results) {
            if (result) {
                ...
                totalLines += (result as any).lineCount || 0;
            }
        }
    ...
    private scoreFile(file: string): number {
        ...
        if (CONFIG_FILES.has(basename)) return 1000;

        // Only score code files
        if (!CODE_EXTENSIONS.has(ext)) return -100;

        // Entry points
        if (/^(index|main|app)\.(ts|js|tsx|jsx)$/.test(basename)) score += 100;

        // API routes (critical for documentation)
        if (relativePath.includes('api/') || relativePath.includes('routes/')) score += 90;
        if (/route\.(ts|js)$/.test(basename)) score += 95;

        // Source directories
        if (relativePath.startsWith('src/')) score += 50;
        if (relativePath.startsWith('app/')) score += 50;
        if (relativePath.startsWith('lib/')) score += 40;
        if (relativePath.startsWith('pages/')) score += 45;

        // Services, controllers, components (architectural)
        if (relativePath.includes('service')) score += 30;
        if (relativePath.includes('controller')) score += 30;
        if (relativePath.includes('component')) score += 20;

        // Hooks (React)
        if (/^use[A-Z]/.test(basename)) score += 25;
    ...
    private async analyzeTechStack(): Promise<TechStackInfo> {
        ...
        try {
            ...
            for (const dep of Object.keys(deps)) {
                if (frameworkMap[dep]) techStack.frameworks.push(frameworkMap[dep]);
                if (dbMap[dep] && !techStack.databases.includes(dbMap[dep])) {
                    techStack.databases.push(dbMap[dep]);
                }
                if (toolMap[dep]) techStack.tools.push(toolMap[dep]);
            }
            ...
            if (hasPnpm) techStack.packageManager = 'pnpm';
            else if (hasYarn) techStack.packageManager = 'yarn';
    ...
    private detectAPIsRegex(content: string, relativePath: string, techStack: TechStackInfo): APIEndpoint[] {
        ...
        while ((match = routeRegex.exec(content)) !== null) {
            apis.push({
                ...
                path: match[2],
                file: relativePath,
                framework: techStack.frameworks.find(f =>
                    ['Express', 'Fastify', 'Hono', 'Koa'].includes(f)
                ) || 'HTTP'
            });
        }
    ...
    private extractPagesApiRoute(filePath: string): string {
        return filePath
            .replace(/^.*pages\//, '/')
            .replace(/\.(ts|js)$/, '')
            .replace(/\[([^\]]+)\]/g, ':$1')
    ...
    private detectPatterns(files: FileNode[], allPaths: string[]): DetectedPattern[] {
    ...
    private generateSummary(analysis: CodebaseAnalysis): string {
        const lines: string[] = [];
