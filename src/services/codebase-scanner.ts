...
export interface DependencyGraph {
    ...
    entryPoints: string[];
    coreModules: string[];  // Most imported files
}
...
export class CodebaseScanner {
    ...
    private limit = pLimit(10);  // Concurrent file processing limit
    ...
    async analyze(onProgress?: ProgressCallback): Promise<CodebaseAnalysis> {
        ...
        const results = await Promise.all(
            sourceFiles.map(file => this.limit(() => analyzeFile(file)))
        );

        analysis.structure = results.filter((n): n is FileNode => n !== null);
    ...
    private prioritizeFiles(files: string[]): string[] {
        ...
        files.forEach(file => {
            ...
            if (CONFIG_FILES.includes(basename)) score += 100;

            // Entry points
            if (basename === 'index.ts' || basename === 'index.js') score += 80;
            if (basename === 'main.ts' || basename === 'main.js') score += 80;
            if (basename === 'app.ts' || basename === 'app.js') score += 80;

            // Source files
            if (relativePath.startsWith('src/') || relativePath.startsWith('src\\')) score += 50;
            if (relativePath.startsWith('lib/') || relativePath.startsWith('lib\\')) score += 40;
            if (relativePath.startsWith('app/') || relativePath.startsWith('app\\')) score += 40;

            // API routes
            if (relativePath.includes('api/') || relativePath.includes('routes/')) score += 60;

            // Tests are lower priority
            if (relativePath.includes('test') || relativePath.includes('spec')) score -= 20;
    ...
    private async analyzeTechStack(): Promise<TechStackInfo> {
        ...
        try {
            ...
            Object.keys(frameworkMap).forEach(dep => {
                if (allDeps[dep]) techStack.frameworks.push(frameworkMap[dep]);
            });
            ...
            Object.keys(toolMap).forEach(dep => {
                if (allDeps[dep]) techStack.tools.push(toolMap[dep]);
            });
    ...
    private hasTypeScriptFiles(files: string[]): boolean {
    ...
    private detectAPIs(sourceFile: SourceFile, relativePath: string, techStack: TechStackInfo): APIEndpoint[] {
    ...
    private extractNextRouteFromPath(filePath: string): string {
    ...
    private extractNextPagesApiRoute(filePath: string): string {
