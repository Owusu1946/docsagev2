 ...
 */
export async function generateLogo(options) {
    const { projectName, style = 'identicon', size = 128, backgroundColor = 'transparent', outputDir = process.cwd() } = options;
 ...
 */
async function generateFallbackLogo(options) {
 ...
 */
export async function generateLogoOptions(projectName) {
    const styles = Object.keys(DICEBEAR_STYLES);
    const seed = encodeURIComponent(projectName.toLowerCase().replace(/[^a-z0-9]/g, ''));
    return styles.map(style => ({
        style,
        url: `https://api.dicebear.com/7.x/${DICEBEAR_STYLES[style]}/svg?seed=${seed}&size=64`
    }));
}
/**
 * Check if project already has a logo
 */
export async function hasExistingLogo(projectDir) {
    const possiblePaths = [
