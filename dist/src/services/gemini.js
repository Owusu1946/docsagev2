...
export class GeminiService {
    ...
    async generateCodeOfConduct(options = { conductType: 'Contributor Covenant', contactEmail: '' }, onProgress) {
        if (onProgress)
            onProgress(`Generating ${options.conductType} Code of Conduct...`);
        const prompt = `
    ...
    async generateReadmeAdvanced(projectName, analysis, options = {}, onProgress) {
        if (onProgress)
            onProgress('Preparing context for Gemini...');
     ...
     */
    async updateReadme(projectName, currentReadme, diff, commitMessage, onProgress) {
        if (onProgress)
            onProgress('Analyzing code changes for documentation updates...');
        const prompt = `
    ...
    `;
        return this.retryOperation(async () => {
            ...
            try {
                ...
                if (onProgress)
                    onProgress(`Applying ${operations.length} documentation updates...`);
